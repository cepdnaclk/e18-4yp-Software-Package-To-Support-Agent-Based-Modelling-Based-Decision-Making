...
print(f"Surface Points: {surface_points}")
for i in range(num_agents):
    agent = MyAgent(unique_id=i, model=mesh)
    random_surface_point = np.random.choice(len(surface_points))  # Random index
    ...
    print(f"{i} : {random_surface_point_coords}")
...
for step in range(num_steps):
    ...
    for agent in mesh._agents:
        print(f"Agent C.Position: {agent.pos}")
        random_surface_point = np.random.choice(len(surface_points))  # Random index
        random_surface_point_coords = surface_points[random_surface_point]  # Extract coordinates
        mesh.move_agent(agent, random_surface_point_coords)
        print(f"After moving pos: {random_surface_point_coords}")
